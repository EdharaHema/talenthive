...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'book_now') {
    ...
    try {
        // Fetch the service ID for "Makeup"
...
// Fetch makeup providers from the businesses table
try {
    $stmt = $pdo->prepare("
    ...
    $error = "Error fetching makeup providers: " . $e->getMessage();
    error_log("Error fetching makeup providers: " . $e->getMessage());
}
?>
...
<head>
  ...
  <style>
    ...
    .hero {
      background-image: url('h_makeup.jpg');
...
<body>
  ...
  <section class="hero">
    <div class="hero-overlay">
      <h1>Discover Skilled Makeup Artists</h1>
      <p>Find talented makeup providers in India for your needs.</p>
      <button class="explore" onclick="document.querySelector('.talent-section').scrollIntoView({ behavior: 'smooth' })">Explore Makeup Artists</button>
    </div>
  </section>
  ...
  <section class="talent-section">
    ...
    <?php elseif (empty($providers)): ?>
      <p>No makeup providers found in this category.</p>
    <?php else: ?>
