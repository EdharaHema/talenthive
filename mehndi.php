...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'book_now') {
    ...
    try {
        // Fetch the service ID for "Mehndi Design"
        $stmt = $pdo->prepare("SELECT id FROM services WHERE name = ?");
        $stmt->execute(['Mehndi Design']);
...
// Fetch mehndi providers from the businesses table
try {
    $stmt = $pdo->prepare("
    ...
    $error = "Error fetching mehndi providers: " . $e->getMessage();
    error_log("Error fetching mehndi providers: " . $e->getMessage());
}
?>
...
<head>
  ...
  <title>TalentHive - Mehndi Design</title>
  <style>
    body {
      margin: 0;
    ...
    .hero {
      background-image: url('h_mehndi.jpg');
...
<body>
  ...
  <section class="hero">
    <div class="hero-overlay">
      <h1>Discover Skilled Mehndi Artists</h1>
      <p>Find talented mehndi providers in India for your needs.</p>
      <button class="explore" onclick="document.querySelector('.talent-section').scrollIntoView({ behavior: 'smooth' })">Explore Mehndi Artists</button>
    </div>
  </section>

  <section class="talent-section">
    <h2>Featured Mehndi Artists</h2>
    <?php if (isset($error)): ?>
      <p style="color: red;"><?php echo htmlspecialchars($error); ?></p>
    <?php elseif (empty($providers)): ?>
      <p>No mehndi providers found in this category.</p>
    <?php else: ?>
