...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'book_now') {
    ...
    try {
        // Fetch the service ID for "Baking"
...
// Fetch bakers from the businesses table
try {
    $stmt = $pdo->prepare("
    ...
    $error = "Error fetching bakers: " . $e->getMessage();
    error_log("Error fetching bakers: " . $e->getMessage());
}
?>
...
<head>
  ...
  <style>
    ...
    .hero {
      background-image: url('h_baking.jpg');
...
<body>
  ...
  <section class="hero">
    <div class="hero-overlay">
      <h1>Discover Skilled Bakers</h1>
      <p>Find talented bakers in India for delicious treats.</p>
      <button class="explore" onclick="document.querySelector('.talent-section').scrollIntoView({ behavior: 'smooth' })">Explore Bakers</button>
    </div>
  </section>
  ...
  <section class="talent-section">
    ...
    <?php elseif (empty($bakers)): ?>
      <p>No bakers found in this category.</p>
    <?php else: ?>
      <div class="talent-grid">
        <?php foreach ($bakers as $baker): ?>
          <div class="talent-card">
            <?php if (!empty($baker['profile_picture'])): ?>
              <img src="uploads/<?php echo htmlspecialchars($baker['profile_picture']); ?>" alt="<?php echo htmlspecialchars($baker['business_name']); ?>">
            <?php else: ?>
              <img src="images/default-profile.jpg" alt="Default Profile">
            <?php endif; ?>
            <h3><?php echo htmlspecialchars($baker['business_name']); ?></h3>
            <p><strong>Location:</strong> <?php echo htmlspecialchars($baker['location']); ?></p>
            <p><?php echo htmlspecialchars($baker['about_me']); ?></p>
            <p><strong>Phone:</strong> <?php echo htmlspecialchars($provider['phone'] ?? 'Not provided'); ?></p>
            <p><strong>Ratings:</strong> <?php echo htmlspecialchars($baker['ratings']); ?>/5</p>
            <div class="button-group">
              <?php if (isset($_SESSION['user']) && $_SESSION['user']['role'] === 'Customer'): ?>
                <button class="book-now" onclick="bookNow(<?php echo htmlspecialchars($baker['id']); ?>)">Book Now</button>
              <?php endif; ?>
            </div>
          </div>
        <?php endforeach; ?>
