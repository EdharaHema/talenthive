...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'book_now') {
    ...
    try {
        // Fetch the service ID for "Crafts"
...
// Fetch crafts providers from the businesses table
try {
    $stmt = $pdo->prepare("
    ...
    $error = "Error fetching crafts providers: " . $e->getMessage();
    error_log("Error fetching crafts providers: " . $e->getMessage());
}
?>
...
<head>
  ...
  <style>
    ...
    .hero {
      background-image: url('h_artandcraft.jpg');
...
<body>
  ...
  <section class="hero">
    <div class="hero-overlay">
      <h1>Discover Skilled Crafts</h1>
      <p>Find talented crafts providers in India for your needs.</p>
      <button class="explore" onclick="document.querySelector('.talent-section').scrollIntoView({ behavior: 'smooth' })">Explore Crafts</button>
    </div>
  </section>
  ...
  <section class="talent-section">
    ...
    <?php elseif (empty($providers)): ?>
      <p>No crafts providers found in this category.</p>
    <?php else: ?>
