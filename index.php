...
// Save search query for logged-in customers
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_search') {
    header('Content-Type: application/json');
    $response = ['success' => false, 'message' => ''];

    if (!isset($_SESSION['user']) || $_SESSION['user']['role'] !== 'Customer') {
        $response['message'] = 'Please log in as a customer to save searches.';
        echo json_encode($response);
        exit;
    }

    $search_query = trim($_POST['skill'] ?? '');
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO recent_searches (customer_id, search_query) VALUES (?, ?)");
        $stmt->execute([$_SESSION['user']['id'], $search_query]);
...
<body>
        ...
        <div class="dropdown user-dropdown">
          ...
          <div class="dropdown-content" id="user-dropdown-content">
            <?php if ($_SESSION['user']['role'] === 'Business'): ?>
              <a href="manage_profile.php" onclick="manageProfile()">Manage Profile</a>
              <a href="view_bookings.php" onclick="viewBookings()">View Bookings</a>
              <a href="logout.php">Logout</a>
            <?php else: ?>
              <a href="view_profile.php" onclick="viewProfile()">View Profile</a>
              <a href="view_bookings.php" onclick="viewBookings()">Bookings</a>
              <a href="view_recent_searches.php" onclick="viewRecentSearches()">Recent Searches</a>
              <a href="logout.php">Logout</a>
            <?php endif; ?>
          </div>
        </div>
  ...
  <section class="hero">
    <div class="hero-overlay">
      ...
      <button class="get-started" onclick="document.getElementById('skill').focus()">Get Started</button>
    </div>
  </section>
  ...
  <script>
    function searchTalent() {
        ...
        fetch('index.php', {
            ...
            body: `action=save_search&skill=${encodeURIComponent(skill)}`
        })
        .then(response => response.json())
        .then(data => {
            console.log(data.message);
            // Redirect to a search results page (you can create this later)
