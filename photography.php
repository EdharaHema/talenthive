...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'book_now') {
    ...
    try {
        // Fetch the service ID for "Photography"
        $stmt = $pdo->prepare("SELECT id FROM services WHERE name = ?");
        $stmt->execute(['Photography']);
...
// Fetch photography providers from the businesses table
try {
    $stmt = $pdo->prepare("
    ...
    $error = "Error fetching photography providers: " . $e->getMessage();
    error_log("Error fetching photography providers: " . $e->getMessage());
}
?>
...
<head>
  ...
  <title>TalentHive - Photography</title>
  <style>
    body {
      margin: 0;
    ...
    .hero {
      background-image: url('h_photography.jpg');
...
<body>
  ...
  <section class="hero">
    <div class="hero-overlay">
      <h1>Discover Skilled Photographers</h1>
      <p>Find talented photography providers in India for your needs.</p>
      <button class="explore" onclick="document.querySelector('.talent-section').scrollIntoView({ behavior: 'smooth' })">Explore Photographers</button>
    </div>
  </section>
  ...
  <section class="talent-section">
    ...
    <?php elseif (empty($providers)): ?>
      <p>No photography providers found in this category.</p>
    <?php else: ?>
